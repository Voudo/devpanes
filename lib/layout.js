import { DIM, RESET, BOLD, REVERSE, PANE_STATE } from './constants.js'
import {
  write, getColumns, getRows, moveTo, clearLine, clearScreen, hideCursor,
  showCursor, truncateToWidth,
} from './terminal.js'

const FOCUS_STYLE = `${BOLD}${REVERSE}`
...
export const createLayout = (apps, processes) => {
  ...
  const restoreCursorPosition = () => {
    if (inputModeActive && inputLine !== null) {
      moveTo(getStatusTopRow() + 2, 5 + inputLine.length)
      showCursor()
    } else {
      moveTo(getRows(), 1)
    }
  ...
  const drawStatusBar = () => {
    ...
    const tabBar = runningEntries.map((entry, index) => {
      const { suffix, isDimmed } = STATE_DISPLAY[entry.state]
      const isFocused = entry.app.key === focusedAppKey
      const displaySuffix = entry.isStopped
        ? ` ${DIM}stopped${RESET}`
        : suffix
      const labelColor = isDimmed || entry.isStopped ? DIM : entry.color
      const focusPrefix = isFocused ? FOCUS_STYLE : ''
      return `${labelColor}${focusPrefix}[${index + 1}] ${entry.app.label}${RESET}${displaySuffix}`
    }).join('   ')
    write(`  ${tabBar}`)

    // Status message / input line
    moveTo(top + 2, 1); clearLine()
    if (inputModeActive && inputLine !== null) {
      write(truncateToWidth(`  ${BOLD}> ${RESET}${inputLine}`, totalColumns))
    ...
    if (runningEntries.length > 0) {
      if (inputModeActive) {
        const entry = processes.get(focusedAppKey)
        const appLabel = entry ? entry.app.label : focusedAppKey
        write(`  ${DIM}esc exit input  (typing sends to ${appLabel})${RESET}`)
      } else {
        const scrollHint = focusedAppKey
          ? 'esc unfocus  ↑↓ scroll  i input  '
          : 'click/tab focus  '
        write(`  ${DIM}${scrollHint}1-${runningEntries.length} show/hide  !@#$ expand  s start  k kill  q quit${RESET}`)
      }
    }
  ...
  const drawInputLine = () => {
    ...
    moveTo(top + 2, 1); clearLine()
    if (inputModeActive && inputLine !== null) {
      write(truncateToWidth(`  ${BOLD}> ${RESET}${inputLine}`, getColumns()))
    }
    restoreCursorPosition()
  }
  ...
  const drawPaneHeader = pane => {
    ...
    const isInputTarget = isFocused && inputModeActive

    const focusStyle = isFocused ? FOCUS_STYLE : ''
    const label = ` ${pane.entry.app.label} `
    const inputTag = isInputTarget ? `${FOCUS_STYLE} INPUT ${RESET}` : ''
    const inputTagLength = isInputTarget ? 7 : 0
    const scrollTag = scrollOffset > 0 ? ` ↑${scrollOffset} ` : ''
    const contentLength = label.length + inputTagLength + scrollTag.length
    const dashCount = Math.max(0, pane.width - contentLength)

    moveTo(pane.headerRow, pane.columnStart)
    write(truncateToWidth(
      `${pane.entry.color}${focusStyle}${label}${RESET}${inputTag}${'─'.repeat(dashCount)}${DIM}${scrollTag}${RESET}`,
      pane.width,
    ))
  }
  ...
  const setInputMode = active => { inputModeActive = active }
  const setInputLine = text => { inputLine = text }
